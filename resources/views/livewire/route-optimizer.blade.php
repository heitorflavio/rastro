<div class="mx-auto w-full max-w-4xl space-y-6">
        ...
        <flux:card>
            <div class="space-y-3">
                <div class="flex items-center gap-2 text-zinc-500">
                    ...
                    <span>Geocodificando endereços e calculando rota… pode levar alguns segundos (Nominatim limita 1
                        req/s).</span>
                </div>
    ...
    @if ($result)
        ...
        <flux:card>
            <div class="space-y-3">
                ...
                <div class="flex flex-wrap gap-2 pt-2">
                    <flux:button as="a" href="{{ $gmaps }}" target="_blank" variant="primary"
                        icon="map-pin">
                        Abrir no Google Maps
                    </flux:button>

                    <flux:button variant="ghost" icon="clipboard" x-data="{ url: @js($gmaps) }"
                        x-on:click="
            ...
            <ol class="space-y-3">
                @foreach ($result['route'] as $pos => $idx)
                    @php
                        ...
                        $label =
                            $pos === 0 ? 'Base' : ($pos === count($result['route']) - 1 ? 'Retorno' : "Parada {$pos}");
                    @endphp
                    <li class="flex items-start gap-3">
                        <flux:badge :color="$isBase ? 'emerald' : 'blue'" size="lg"
                            class="shrink-0 w-10 justify-center">
                            {{ $isBase ? '🏠' : $pos }}
                        </flux:badge>
                        <div>
        ...
        <flux:card>
            ...
            <div wire:ignore x-data x-init="const data = @js($result['history']);
            ...
            ctx.stroke();">
                <canvas x-ref="canvas" width="900" height="240"
                    class="w-full rounded-lg border border-zinc-200 dark:border-zinc-700"></canvas>
            </div>
        </flux:card>
    @endif
</div>
